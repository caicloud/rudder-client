...
func convertObjectToDeploy(obj runtime.Object) (*appsv1.Deployment, error) {
...
func convertDeployToController(dp *appsv1.Deployment) (*universal.Controller, error) {
	spec := dp.Spec
...
func convertDeployStrategy(dpStrategy appsv1.DeploymentStrategy) universal.Strategy {
	ret := universal.Strategy{
		Type: string(dpStrategy.Type),
	}
	if dpStrategy.Type == "" {
		ret.Type = string(appsv1.RollingUpdateDeploymentStrategyType)
	}
